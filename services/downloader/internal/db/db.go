...
func Init() error {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if openErr != nil {
			...
			if poolErr != nil {
				...
				if err == nil {
					...
					logger.Log.Infof("[Downloader Service] Connected to PostgreSQL")
					return nil
				}
				pool.Close()
			}
		...
		logger.Log.Warnf("[Downloader Service] PostgreSQL not ready (attempt %d/%d): %v", attempt, maxAttempts, err)
